...
export class OrderService {
    ...
    getCouponPage(page = 1, size = 10): Observable<any> {
        return this.http.get(`${disUrl}/coupon/list?page=${page}&size=${size}`).pipe();
    }

    getCoupon():Observable<any>{
        return this.http.get(`${disUrl}/coupon/alllist`);
    }
    ...
    deleteCoupon(code:string): Observable<any>{
        console.log("service")
        return this.http.post(`${disUrl}/delete/coupon/${code}`,null);
    }
}
